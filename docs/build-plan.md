...
shared/
  ui/ Sheet, SheetHeader, SheetHost (stack, fixed,
      history-synced back-button-closes-sheet — M6),
      ConfirmDialog, AlertDialog, Chip, BottomNav,
      SetBlock, SetTable, Skeleton (M6, loading.tsx use),
      icons.tsx (incl. StrengthIcon/
...
### M6 — Polish ✅ DONE
- [x] Back-button-closes-sheet: `shared/ui/SheetHost.tsx` now pushes a no-op `history.pushState` entry on every `push()` and listens for `popstate` to pop the top sheet. **Bug found and fixed while building this**: a UI-triggered `pop()` (X button, backdrop click, save/discard) updates the stack immediately *and* calls `history.back()` to keep the URL in sync — but that `back()` still fires its own `popstate` a moment later, and the handler couldn't tell that apart from a real back-button press, so it double-popped (closing two sheets, e.g. the whole routine editor, when only the exercise picker on top should've closed). Fixed with a `pendingProgrammaticPops` counter the handler consumes instead of re-decrementing the stack. `closeAll()` uses the same counter, since `history.go(-n)` fires exactly one `popstate` regardless of how many entries it skips. Verified via scripted Playwright: single back-press on a nested picker-over-editor stack closes only the picker; the routine draft underneath survives.
- [x] **Real bug found and fixed, unrelated to the above**: signing up or signing in left the browser genuinely stuck on the `/login` URL (`window.location.pathname` confirmed, not just a Playwright artifact) while the home page rendered correctly — because `BottomNav` calls `usePathname()` and hides itself on `/login`, the nav bar silently disappeared after every single sign-in until a manual reload. Root cause: `proxy.ts`'s "authenticated user hits `/login` → redirect to `/`" branch races with Convex Auth's own post-sign-in router-cache invalidation (a server action that deletes a dummy cookie to force a soft refresh) — the redirect's content wins but its history/URL commit gets lost. Fixed by having `LoginForm.tsx` call `router.replace("/")` itself right after `signIn()` resolves, instead of relying on the middleware redirect to carry the navigation. `proxy.ts` is unchanged and still correctly handles a *direct* hard navigation to `/login` while already authenticated (e.g. a stale bookmark).
- [x] `loading.tsx` skeletons audited per route — all four (`/`, `/routines/[id]`, `/sessions/[id]`, `/exercises/[name]`) replaced with layout-shaped `animate-pulse` skeletons (new `shared/ui/Skeleton.tsx` primitive) matching each route's real content shape, instead of plain "Loading…" text.
- [x] Empty states audited against the full spec — checked every one named in the prototype (`docs/workout-prototype-app.md`) against the live app in a real browser: "No routines yet." / "Empty folder." / "No exercises yet." (routine editor and live logging) / "No records yet." / "No history yet." / "No data yet." (Progress) / the zero-sessions greeting copy / the routine trend chart's "Do it again to see a trend." single-session state. All matched; no gaps found. Statistics has no dedicated brand-new-user empty copy in the prototype either — it just renders zeroed tiles/targets/an uncollapsed radar, confirmed correct in-browser.
- [x] Responsive breakpoints verified at 768px/1000px+ against actual rendered output (scripted Playwright, not just reading class names): nav pill floats correctly at `md:`, the workout home's two-column `lg:grid` kicks in correctly ≥1024px, Statistics/Progress reflow correctly at both widths. (Two apparent rendering gaps turned out to be the test script's wait being shorter than a Convex query's first resolve — reproduced and ruled out with a longer wait; not app bugs.)
- [x] Tailwind lint suggestions cleanup: mechanically converted every `<spacing-utility>-[Npx]` arbitrary value (width/height/padding/margin/gap/inset — anything on Tailwind v4's numeric `--spacing` scale) to its canonical `N.NN` decimal class (e.g. `px-[13px]` → `px-3.25`, `h-[22px]` → `h-5.5`) across all 29 affected files. Spot-verified in a real browser that the decimal classes resolve to the exact original pixel values (`getComputedStyle`). Deliberately left `text-[Npx]` (font-size has no numeric Tailwind scale) and `rounded-[Npx]` (this app's `--radius-card` token is the only custom radius; 9/10/11/22px don't match Tailwind's named radius steps) as arbitrary values — there's no canonical class for either.
- [x] Full combined click-through pass across every tab in one session (folder create → grouped routine create → cardio log → strength log with real weights → Workout tab → Progress tab → Statistics tab all 3 sub-tabs, now with real data for the first time instead of each M3–M5 pass's isolated empty/single-item states → session detail → sign out → sign back in). Zero console errors; all stats (set/rep counts, muscle-group dedup, volume, radar) cross-checked by hand against what was actually logged and matched.
...
3. All planned milestones (M0–M6) are done. There's no next-unchecked-milestone to auto-resume at — check in with whoever's picking this up for what's next (new feature, deploy, etc.) rather than assuming.
